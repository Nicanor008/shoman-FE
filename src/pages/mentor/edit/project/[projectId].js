...
function NewProject(props) {
  ...
  useEffect(() => {
    setLoading(true)
    GetData(`/contents/${projectId}`).then(content => {
      if (content.status === "error") window.location.href = "/mentor/project"
      setLoading(false)
      setData({
        ...data,
        trackId: content.project.trackId,
        projectDescription: content.project.projectDescription,
        topic: content.project.topic,
        team: content.project.team,
        deadline: content.project.deadline,
        duration: content.project.duration,
        showmanAccess: content.project.showmanAccess,
      })
    }).catch(error => {
      setLoading(false)
      if (error) window.location.href = "/mentor/project"
  ...
  //   submit data
  const submitProject = () => {
    PostWithToken(`/projects/${projectId}`, "patch", data)
      .then((response) => {
        if (!response.message) {
